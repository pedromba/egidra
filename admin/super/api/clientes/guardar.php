...
if ($id > 0) {
    $stmt = $conexion->prepare(
        "UPDATE clientes SET nombre=?, iniciales=?, sector=?, logo=?, descripcion=?, activo=?
         WHERE id_cliente=?"
    );
    $stmt->bind_param('sssssii', $nombre, $ini, $sector, $logo, $desc, $activo, $id);
} else {
    $stmt = $conexion->prepare(
        "INSERT INTO clientes (nombre, iniciales, sector, logo, descripcion, activo)
         VALUES (?, ?, ?, ?, ?, ?)"
    );
    $stmt->bind_param('sssssi', $nombre, $ini, $sector, $logo, $desc, $activo);
}
