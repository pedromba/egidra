...
if ($id > 0) {
    // UPDATE: 10 params → s(titulo) i(cat) i(cli) s(ubic) i(ano) s(desc) s(img) i(dest) i(act) i(id)
    ...
    $stmt->bind_param('siisissiiii', $titulo, $cat_id, $cli_id, $ubicacion, $ano, $desc, $imagen, $dest, $activo, $id);
} else {
    // INSERT: 9 params → s(titulo) i(cat) i(cli) s(ubic) i(ano) s(desc) s(img) i(dest) i(act)
    $stmt = $conexion->prepare(
    ...
    $stmt->bind_param('siisisiii', $titulo, $cat_id, $cli_id, $ubicacion, $ano, $desc, $imagen, $dest, $activo);
}
