...
if ($id > 0) {
    ...
    $stmt->bind_param('ssssiis', $nombre, $slug, $icono, $desc, $orden, $activo, $id);
} else {
    $stmt = $conexion->prepare(
...
if ($stmt->execute()) {
    ...
    registrar_log($conexion, $_SESSION['user_id'], $id > 0 ? 'EDITAR' : 'CREAR', ($id > 0 ? 'Editado' : 'Creado') . ': ' . ($nombre ?? 'registro'), 'categorias_servicios', $nuevo_id);
